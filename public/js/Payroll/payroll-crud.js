/** Payroll CRUD */
class PayrollCRUD extends CRUDManager {
    constructor() {
        super({
            entityName: 'Payroll',
            queryName: 'allPayrolls',
            createMutation: 'createPayroll',
            updateMutation: 'updatePayroll',
            deleteMutation: 'deletePayroll',
            fields: `id staff_id period gross_salary deductions net_salary staff { id user { name } }`,
            relatedFields: {}
        });
        this.loadStaffs();
    }
    ...
    displayData(data) {
        ...
        data.forEach(item => {
            ...
            row.innerHTML = `
                ...
                <td class="px-6 py-4 text-sm">Rp ${parseInt(item.gross_salary).toLocaleString()}</td>
                <td class="px-6 py-4 text-sm">Rp ${parseInt(item.deductions).toLocaleString()}</td>
                <td class="px-6 py-4 text-sm">Rp ${parseInt(item.net_salary).toLocaleString()}</td>
    ...
    async create(data) {
        try {
            const mutation = `mutation { createPayroll(input: { staff_id: ${data.staff_id}, period: "${data.period}", gross_salary: ${data.gross_salary}, deductions: ${data.deductions}, net_salary: ${data.net_salary} }) { id } }`;
    ...
    async update(data) {
        try {
            const mutation = `mutation { updatePayroll(id: ${this.currentId}, input: { staff_id: ${data.staff_id}, period: "${data.period}", gross_salary: ${data.gross_salary}, deductions: ${data.deductions}, net_salary: ${data.net_salary} }) { id } }`;
            const response = await fetch('/graphql', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '' }, body: JSON.stringify({ query: mutation }) });
            const result = await response.json();
            if (result.errors) this.showNotification('Failed', 'error');
...
document.addEventListener('DOMContentLoaded', function() { window.crud = new PayrollCRUD(); crud.load(1); });
