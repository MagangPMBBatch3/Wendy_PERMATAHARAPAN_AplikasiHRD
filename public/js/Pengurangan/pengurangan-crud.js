/** Pengurangan CRUD */
class PenguranganCRUD extends CRUDManager {
    constructor() {
        super({
            entityName: 'Pengurangan',
            queryName: 'allPengurangans',
            createMutation: 'createPengurangan',
            updateMutation: 'updatePengurangan',
            deleteMutation: 'deletePengurangan',
    ...
    async create(data) {
        try {
            const mutation = `mutation { createPengurangan(input: { staff_id: ${data.staff_id}, type: "${data.type}", amount: ${data.amount}, start_date: "${data.start_date}", end_date: "${data.end_date}" }) { id } }`;
    ...
    async update(data) {
        try {
            const mutation = `mutation { updatePengurangan(id: ${this.currentId}, input: { staff_id: ${data.staff_id}, type: "${data.type}", amount: ${data.amount}, start_date: "${data.start_date}", end_date: "${data.end_date}" }) { id } }`;
            const response = await fetch('/graphql', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '' }, body: JSON.stringify({ query: mutation }) });
            const result = await response.json();
            if (result.errors) this.showNotification('Failed', 'error');
...
document.addEventListener('DOMContentLoaded', function() { window.crud = new PenguranganCRUD(); crud.load(1); });
