/** Permintaan CRUD */
class PermintaanCRUD extends CRUDManager {
    constructor() {
        super({
            entityName: 'Permintaan',
            queryName: 'allPermintaans',
            createMutation: 'createPermintaan',
            updateMutation: 'updatePermintaan',
            deleteMutation: 'deletePermintaan',
            fields: `id staff_id type status reason submission_date staff { id user { name } }`,
            relatedFields: {}
        });
        this.loadStaffs();
    }
    ...
    displayData(data) {
        ...
        data.forEach(item => {
            ...
            row.innerHTML = `
                ...
                <td class="px-6 py-4 text-sm">${item.submission_date || '-'}</td>
    ...
    async create(data) {
        try {
            const mutation = `mutation { createPermintaan(input: { staff_id: ${data.staff_id}, type: "${data.type}", status: "${data.status}", reason: "${data.reason || ''}", submission_date: "${data.submission_date}" }) { id } }`;
    ...
    async update(data) {
        try {
            const mutation = `mutation { updatePermintaan(id: ${this.currentId}, input: { staff_id: ${data.staff_id}, type: "${data.type}", status: "${data.status}", reason: "${data.reason || ''}", submission_date: "${data.submission_date}" }) { id } }`;
            const response = await fetch('/graphql', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '' }, body: JSON.stringify({ query: mutation }) });
            const result = await response.json();
            if (result.errors) this.showNotification('Failed', 'error');
...
document.addEventListener('DOMContentLoaded', function() { window.crud = new PermintaanCRUD(); crud.load(1); });
