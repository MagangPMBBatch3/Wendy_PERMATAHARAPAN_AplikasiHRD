...
@section('content')
    <div class="bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 p-6 rounded-2xl shadow-2xl w-full min-h-screen">
        <h1 class="text-3xl font-bold mb-6 text-white text-center">Data Payroll</h1>
        ...
        <div class="flex justify-between mb-6">
            <input
                type="text"
                id="searchInput"
                placeholder="Cari ID, Periode, atau Status..."
        ...
        @include('components.payroll.modal-add')

        {{-- Include Modal Edit --}}
        @include('components.payroll.modal-edit')

        {{-- Script --}}
        <script src="{{ asset('js/Payroll/payroll.js') }}"></script>
        <script src="{{ asset('js/Payroll/payroll-create.js') }}"></script>
        <script src="{{ asset('js/Payroll/payroll-edit.js') }}"></script>
    </div>
@endsection
