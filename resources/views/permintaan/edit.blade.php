...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600">Update request information</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form id="editRequestForm" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="staff_id" name="staff_id" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    ...
                    @foreach($staffs as $staff)
                        <option value="{{ $staff->id }}" {{ $permintaan->staff_id == $staff->id ? 'selected' : '' }}>{{ $staff->nama }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="tipe" name="tipe" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <option value="cuti" {{ $permintaan->tipe == 'cuti' ? 'selected' : '' }}>Cuti (Leave)</option>
                    <option value="izin" {{ $permintaan->tipe == 'izin' ? 'selected' : '' }}>Izin (Permission)</option>
                    <option value="lembur" {{ $permintaan->tipe == 'lembur' ? 'selected' : '' }}>Lembur (Overtime)</option>
                </select>
            ...
            <div>
                ...
                <input type="date" id="tanggal" name="tanggal" required value="{{ $permintaan->tanggal }}"
            ...
            <div id="timeRangeContainer" class="hidden space-y-4">
                <div>
                    ...
                    <input type="time" id="waktu_mulai" name="waktu_mulai" value="{{ $permintaan->waktu_mulai ? substr($permintaan->waktu_mulai, 0, 5) : '' }}"
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                ...
                <div>
                    ...
                    <input type="time" id="waktu_selesai" name="waktu_selesai" value="{{ $permintaan->waktu_selesai ? substr($permintaan->waktu_selesai, 0, 5) : '' }}"
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500">
            ...
            <div>
                ...
                <textarea id="keterangan" name="keterangan" rows="4" required
                          class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                          placeholder="Provide details about your request...">{{ $permintaan->keterangan }}</textarea>
